...
hAzzle.event = {
    ...
    add: function (elem, types, handler, data, selector) {
        ...
        if (!(eventHandle = eD.handle)) {

            eventHandle = eD.handle = Listener();
        }
    ...
    remove: function (elem, types, handler, selector, mappedTypes) {
        ...
        var j, origCount, tmp,
            events, t, handleObj,
            special, handlers, type, namespaces, origType,
            eD = hAzzle.hasData(elem) && hAzzle.data(elem);
        ...
        while (t--) {
            ...
            if (origCount && !handlers.length) {
                if (!special.shutdown ||
                    special.shutdown.call(elem, namespaces, eD.handle) === false) {

                    if (elem.removeEventListener) {
                        elem.removeEventListener(type, eD.handle, false);
                    }
                }
                delete events[type];
            }
