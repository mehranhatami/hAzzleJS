 ...
 * Get hAzzle event id
 *
 * @param {Object} el The element to get hAzzle event id from
 *
 * @return {Number}
 */
  ...
  if (parts.ns.length) {
    for (event in c[id]) {
      ...
      for (var i = 0, l = tmp.length; i < l; i++) {
        if (tmp[i] && ns.length && tmp[i].ns.length && hAzzle.inArray(ns, tmp[i].ns.split(' ')) !== -1) {
          handlers.push(tmp[i]);
        }
      }
    }
...
function createEventHandler (el, event, callback, _callback) {
  ...
  var fn = function (event) {
    ...
    var data = event.data;
  //  if (hAzzle.isString(data) && /^[\[\{]/.test(data)) data = hAzzle.parseJSON(event.data);
...
function addEvent (el, events, callback, selector) {
  ...
  if (hAzzle.isString(selector)) {
    ...
    fn = function () {
      return (function (el, callback, selector) {
        return function (e) {
          var match = hAzzle(el).find(e.target || e.srcElement);
          match = match.get(0) === el ? match.find(selector) : match;
          if (match.is(selector)) {
            var event = hAzzle.extend(createProxy(e), {
  ...
  hAzzle.each(events.split(/\s/), function (index, event) {
    var parts = getEventParts(event);
...
function removeEvent (el, events, callback, selector) {
  ...
  if (callback === undefined && hAzzle.isFunction(selector)) {
    callback = selector;
    selector = undefined;
  }
    ...
    for (var i = 0; i < handlers.length; i++) {
      if (testEventHandler(parts, callback, selector, handlers[i])) {
        ...
        if (el.removeEventListener) {
          ...
          var name = 'on' + event;
          if (hAzzle.isString(el[name])) el[name] = null;
...
hAzzle.fn.extend({
  ...
  trigger: function (event, data, _el) {
    return this.each(function (i, el) {
      ...
      event = hAzzle.Event(event)
      event.data = data || {};

      if (hAzzle.isString(event.data) && !hAzzle.isString(data) && JSON.stringify) {
                ...
                if (ev.eventName && ev.srcElement._eventId) {
                  ...
                  if (handlers.length) {
                    for (var i = 0, l = handlers.length; i < l; i++) {
                      if (hAzzle.isFunction(handlers[i])) handlers[i](ev);
                    }
                  }
                }
              };
      ...
      if (!event.isPropagationStopped()) {
        ...
        if (parent && parent._eventId > 0) {
          // hAzzle use `liveTarget` instead of creating a own Event object that modifies `target` property.
          event.liveTarget = el;
          hAzzle(parent).trigger(event, data);
        } else {
