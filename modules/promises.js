...
hAzzle.when = function(subordinate /* , ..., subordinateN */ ) {
    ...
    if (length > 1) {
        ...
        for (; i < length; i++) {
            if ((resolveValue = resolveValues[i]) && hAzzle.isFunction(resolveValue.then)) {
                resolveValue.then(
                    updateFunc(i, resolveContexts, resolveValues),
                    deferred.reject
                );

                if (hAzzle.isFunction(resolveValue.progress)) {
    ...
    return promised.promise ? promised.promise() : hAzzle.promises(function(newDefer) {
        promised.then(newDefer.resolve, newDefer.reject);
    }).promised();
}